...
export default function DashboardPage() {
		...
		async function fetchProfile() {
			...
			try {
				...
				if (!res.ok) {
					...
					// Simply set the entire returned object to state:
					setProfile(data);
					// For debugging, you can see exactly what's returned:
	...
	useEffect(() => {
		...
		async function fetchPosts() {
			try {
				// Pass a very high limit to load all posts.
	...
	// ───────────────────────────────────────────────
	// 3a) Profile picture update handler.
	// ───────────────────────────────────────────────
	const handleProfilePictureChange = (e) => {
		const file = e.target.files[0];
		if (file) {
			// TODO: Implement file upload logic to update profile picture
				...
				<div className="md:w-1/3 space-y-4">
					<form onSubmit={handleSubmit}>
						...
						<button
							type="submit"
							disabled={updating}
							className="mt-2 w-full bg-black text-white px-4 py-2 rounded hover:bg-green-700"
						>
										...
										<div className="flex flex-col border-b border-gray-200 pb-6 hover:bg-gray-50 transition px-2">
											...
											<div className="grid grid-cols-[1fr_auto] gap-4 items-start mb-2">
												<div>
													...
													{/* Description hidden on mobile for better layout */}
